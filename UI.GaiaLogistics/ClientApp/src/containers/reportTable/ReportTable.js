import subDays from 'date-fns/subDays';
import MuiBox from '@mui/material/Box';
import MuiStack from '@mui/material/Stack';
import MuiPaper from '@mui/material/Paper';
import MuiTable from '@mui/material/Table';
import MuiTableBody from '@mui/material/TableBody';
import MuiTableCell from '@mui/material/TableCell';
import MuiTableContainer from '@mui/material/TableContainer';
import MuiTablePagination from '@mui/material/TablePagination';
import MuiTableHead from '@mui/material/TableHead';
import MuiTableRow from '@mui/material/TableRow';
import { ReportTableRow } from './reportTableRow';
import { DatePicker } from '../../components/datePicker';
import { Dropdown } from '../../components/dropdown';
import { Spinner } from '../../components/spinner';
import { useQueryReport } from '../../hooks/useQueries';
import useDateFormatter from '../../hooks/useDateFormatter';
import { UsersDropdownContainer } from '../usersDropdown';
import {
  DestinationBranchesDropdownContainer,
  OriginBranchesDropdownContainer,
} from '../branchDropdown';

const DEFAULT_FILTERS = {
  causeType: 'Transfer',
  originId: '',
  destinationId: '',
  userId: '',
  depositFilter: 'full',
  sizeLimit: 10,
  currentPage: 1,
};

const ReportTableContainer = () => {
  const { getRFC3339 } = useDateFormatter();
  const { data, status, error, filters, setFilters } = useQueryReport({
    ...DEFAULT_FILTERS,
    from: getRFC3339(subDays(new Date(), 6)),
    to: getRFC3339(new Date()),
  });

  const handleChangeFromDate = event => {
    setFilters(prevState => ({ ...prevState, from: getRFC3339(event.target.value) }));
  };

  const handleChangeToDate = event => {
    setFilters(prevState => ({ ...prevState, to: getRFC3339(event.target.value) }));
  };

  const handleChangeUser = value => {
    setFilters(prevState => ({ ...prevState, userId: value }));
  };

  const handleChangeDestinationBranch = value => {
    setFilters(prevState => ({ ...prevState, destinationId: value }));
  };

  const handleChangeOriginBranch = value => {
    setFilters(prevState => ({ ...prevState, originId: value }));
  };

  const handleChangeDestinationFilter = value => {
    setFilters(prevState => ({ ...prevState, depositFilter: value }));
  };

  const handleChangePage = (event, newPage) => {
    setFilters(prevState => ({ ...prevState, currentPage: newPage + 1 }));
  };

  const handleChangeRowsPerPage = (event) => {
    setFilters(prevState => ({ ...prevState, sizeLimit: parseInt(event.target.value, 10), currentPage: 1 }));
  };

  return (
    <>
      <Spinner loading={['loading', 'refetching'].includes(status)} label="Cargando datos..." />
      {status === 'error' && <div>{error.message}</div>}
      <MuiStack direction="row" spacing={2} sx={{ my: 2 }}>
        <DatePicker
          label="Desde"
          aria-label="from date"
          id="fromDate"
          name="fromDate"
          value={new Date(filters.from)}
          onChange={handleChangeFromDate}
        />
        <DatePicker
          label="Hasta"
          aria-label="to date"
          id="toDate"
          name="toDate"
          value={new Date(filters.to)}
          onChange={handleChangeToDate}
        />
        <UsersDropdownContainer keyField="email" value={filters.userId} onChange={handleChangeUser} />
      </MuiStack>
      <MuiStack direction="row" spacing={2} sx={{ my: 2 }}>
        <OriginBranchesDropdownContainer keyField="code" value={filters.originId} onChange={handleChangeOriginBranch} />
        <DestinationBranchesDropdownContainer keyField="code" value={filters.destinationId} onChange={handleChangeDestinationBranch} />
        <Dropdown
          sx={{ width: 200 }}
          label="Mostrar ubicaciones"
          ariaLabel="destination-filter"
          value={filters.destinationFilter}
          items={[{ id: 'full', text: 'Todo' }, { id: 'union', text: 'Intersección' }]}
          onChange={handleChangeDestinationFilter}
        />
      </MuiStack>
      {status === 'success' && data && (
        <MuiBox sx={{ my: 2 }}>
          <MuiTableContainer component={MuiPaper}>
            <MuiTable aria-label="collapsible table">
              <MuiTableHead>
                <MuiTableRow>
                  <MuiTableCell />
                  <MuiTableCell align="left">Origen</MuiTableCell>
                  <MuiTableCell align="left">Destino</MuiTableCell>
                  <MuiTableCell align="left">Responsable</MuiTableCell>
                  <MuiTableCell align="left">Fecha</MuiTableCell>
                </MuiTableRow>
              </MuiTableHead>
              <MuiTableBody>
                {data.results.length > 0 && data.results.map(row => (
                  <ReportTableRow key={row.id} row={row} />
                ))}
                {data.results.length === 0 && (
                  <MuiTableRow>
                    <MuiTableCell component="th" scope="row" align="center" colSpan={5}>
                      No hay datos para mostrar
                    </MuiTableCell>
                  </MuiTableRow>
                )}
              </MuiTableBody>
            </MuiTable>
          </MuiTableContainer>
          <MuiTablePagination
            rowsPerPageOptions={[5, 10, 25]}
            component="div"
            count={data.total}
            rowsPerPage={filters.sizeLimit}
            page={filters.currentPage - 1}
            onPageChange={handleChangePage}
            onRowsPerPageChange={handleChangeRowsPerPage}
            labelDisplayedRows={({ from, to, count }) =>
              `${from}-${to} de ${count !== -1 ? count : `más de ${to}`}`
            }
            labelRowsPerPage="Filas por página:"
          />
        </MuiBox>
      )}
    </>
  );
};

export default ReportTableContainer;
